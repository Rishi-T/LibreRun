...
class _FilelistHandler:
    ...
    def run(self, optimize: bool = False):
        ...
        for nickname, files in groups.items():
            ...
            self.con.info(
                f"Written: {C.BOLD}{fl_path}{C.RESET}  "
        ...
        # Treat 0 filelists generated as a fatal error
        if len(generated_filelists) == 0:
            self.con.fatal(
                f"\n{C.FAIL}Filelist generation failed.{C.RESET}\n"
                f"0 filelist(s) written. Check that rtl_source_dirs points to valid directories with RTL files."
            )

        self.con.info(
            f"\n{C.PASS}Filelist generation complete.{C.RESET}\n"
