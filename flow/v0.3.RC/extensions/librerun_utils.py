...
class C:
    ...
    DIM         = "\033[2m"
    BOLD        = "\033[1m"
    CMD         = "\033[38;5;245m"  # light grey for commands
...
def run_hook(hook_name: str, script_str: str, cwd: Path,
    ...
    width = 75
    console.msg(f"\n{C.STRUCT}{'~' * 17} Starting {hook_name} {'~' * (width - 19 - len(hook_name))}{C.RESET}\n")
    ...
    console.msg(f"\n{C.STRUCT}{'~' * 17}  Exiting {hook_name} {'~' * (width - 19 - len(hook_name))}{C.RESET}\n")
