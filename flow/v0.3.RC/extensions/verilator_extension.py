...
class _VerilatorHandler:
    ...
    def run_lint(self):
        ...
        self.con.info(f"Lint command : {C.CMD}{' '.join(cmd)}{C.RESET}\n")
    ...
    def run_compile(self):
        ...
        self.con.info(f"Compile command : {C.CMD}{' '.join(cmd)}{C.RESET}")
    ...
    def run_sim(self, binary: Path, args, run_dir: Path):
        ...
        self.con.info(f"Sim run directory : {C.CMD}{run_dir}{C.RESET}")

        pre_run_script = cfg_get(simopts, "pre_run_script", default="").strip()
        if pre_run_script:
            flow_args = {
                "--project-root": run_dir.parents[3],
                "--config-name":  self.config_name,
                "--run-number":   run_dir.name.split("_")[-1],
            }
        ...
        self.con.blank()
        self.con.info(f"Sim command       : {C.CMD}{' '.join(cmd)}{C.RESET}")
